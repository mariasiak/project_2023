    ...
    do {
        ...
        for(int i=0;i<number_of_queries;i++) {
            ...
            for(int graph_search_nn_index:graph_search_nn_indexes) {
                ...
                std::cout<<"Distance from query: "<<VecMath::dist_o(query_o,DataHandler::get_data_point_at_o(graph_search_nn_index))<<std::endl;
            }    

            // clock and run exact KNN
            auto start_time_true = std::chrono::high_resolution_clock::now();        

            std::vector<int> exact_nn_indexes = NearestNeighbor::query_KNN_o(query_o,N);
            ...
            for(int j=1;j<=N;j++) {
                ...
                // Project 3 - distances calculated on original dimensions
                out_stream<<"distanceApproxmiate: "<<VecMath::dist_o(query_o,DataHandler::get_data_point_at_o(graph_search_nn_indexes[j-1]))<<std::endl;
