...
void Clustering::k_means_pp_init() {
    ...
    for(int i=1;i<k;i++) {
        ...
        for(int j=0;j<dataset_size;j++) {
            ...
            int nearest_neighbor_index = NearestNeighbor::run(jth_point,centroid_indexes);
            Datapoint nearest_centroid = DataHandler::get_data_point_at(nearest_neighbor_index);

            // calculatea distance from it
            int distance_from_nearest_centroid = VecMath::dist(jth_point,nearest_centroid);
...
std::vector<double> Clustering::get_silhouette_scores() {
    ...
    for(int k=0;k<num_of_clusters;k++) {
        ...
        for(int i=0;i<cluster_size;i++) {
            ...
            double ai=0;
            Datapoint ith_point=DataHandler::get_data_point_at_o(clusters[k].point_indexes[i]);
            ...
            for(int j=0;j<cluster_size;j++) {
                if(i!=j) {
                    ...
                    Datapoint jth_point=DataHandler::get_data_point_at_o(clusters[k].point_indexes[j]);
                    ai+=VecMath::dist_o(ith_point,jth_point);
                }
            }
            ...
            for(int j=0;j<nearest_cluster_size;j++) {
                ...
                Datapoint jth_point=DataHandler::get_data_point_at_o(clusters[nearest_cluster_index].point_indexes[j]);
                bi+=VecMath::dist_o(ith_point,jth_point);
            }
